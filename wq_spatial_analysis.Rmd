---
title: "Water Quality Spatial Analysis"
...
#LAGOSNE::lagosne_get(dest_folder = LAGOSNE:::lagos_path(),overwrite=T)
...
#view(chla_secchi_200)

ggplot(chla_secchi_200, aes(x=chla, y=secchi))+
  geom_point()+
  labs(x= 'Chlorophyll A', y= 'Secchi Disk Depth')+
  theme_few()
...
The increase in Chlorophyll A means an increase of algae in the system. An increase in algae decreases the water clarity, and hence the secchi disk depth would decrease, because the last visible disk would be closer to the surface. The less Chlorophyll A, the clearer the water.
...
states_nutr <- st_join(states, spatial_nutr,
                         by='lagoslakeid')
...
secchi_200_mean <- secchi_200 %>%
  group_by(lagoslakeid) %>%
  summarise(mean_secchi = mean(secchi))

spatial_secchi_200 <- inner_join(spatial_lakes, secchi_200_mean, 
           by='lagoslakeid')

mapview(spatial_secchi_200, zcol='mean_secchi')
